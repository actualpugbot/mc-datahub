...
workspace/
  ...
  datasets/<version>/
    ...
    tags.json
    loot-tables.json
    fishing-odds.json   Fishing loop + exact luck x open-water odds grid + wait-time model
    loot-odds.json      Exact odds for bartering, archaeology, gifts, mob drops, chests
    advancements.json
    translations.json
    textures.json
...
- `ore-generation.json`: exact configured-feature placement-attempt curves for every player-facing ore, joined from configured features, placed features, biome feature lists, and noise-setting height bounds. It includes dimension/biome contexts, vein size, replaceable tags, air-exposure discard chance, the mathematical peak, and a practical recommendation that avoids the bedrock-floor band. These are placement attempts rather than guaranteed ore-block counts; terrain shape and vein placement remain seed-dependent.
...
- `fishing-odds.json`: everything a "what are the odds when I fish?" page needs, in one read. `mechanics` is the whole fishing loop parsed out of `FishingHook.java`, `FishingRodItem.java`, `EnchantmentHelper.java`, `LootPoolSingletonContainer.java` and the `lure`/`luck_of_the_sea` enchantment JSON: the `timeUntilLured` roll (100-600 ticks) and how `lureSpeed` is subtracted from it, the per-tick `fishingSpeed` decrement with its rain (+1 at 25%) and obstructed-sky (-1 at 50%) modifiers, the tease-splash chance tiers, the approach (`timeUntilHooked`, 20-80) and bite (`nibble`, 20-40) windows, the 4-layer 5x5 open-water scan with its block-classification rules, every luck source (Luck of the Sea's `fishing_luck_bonus`, the Luck/Bad Luck effects, the `minecraft:luck` attribute, and the `withLuck(this.luck + owner.getLuck())` call that seeds the loot context), the `max(floor(weight + quality * luck), 0)` weight formula, and the XP/rod-damage rewards. `oddsGrid` is the exact per-item odds for every integer luck in `[-4, 10]` x open/closed water, with category shares, full count distributions, `set_damage` wear fractions and `enchant_with_levels` metadata. `scenarios` names the 26 combinations a preset UI shows (unenchanted, Luck of the Sea I-III, Luck/Bad Luck potions, and the enchantment-plus-potion stacks) and points each at its grid cell. `timing` derives the expected seconds per catch for Lure 0-3 across all four weather/sky combinations by solving the countdown recurrence exactly, plus catches and XP per hour. Anything that cannot be derived becomes a `warnings` entry instead of a guess. See [docs/CONSUMING_LOOT_ODDS.md](docs/CONSUMING_LOOT_ODDS.md)
- `loot-odds.json`: the same exact-probability engine pointed at everything else worth a guide page — `bartering` (every piglin trade with its probability, expected count per gold ingot, and the ingots needed per item), `archaeology` (all six brushable suspicious-block tables), `gifts` (cat morning gift, sniffer digging, chicken lay, panda sneeze, armadillo shed, turtle grow, and all fourteen hero-of-the-village gifts), `mobDrops` (every `minecraft:entities/*` table at Looting 0-3, with a second set of contexts wherever `minecraft:killed_by_player` actually changes the answer, and identical contexts collapsed into one cell), and `chests` (all 56 `minecraft:chests/*` tables at luck 0). Every row carries the probability of at least one, the expected count, the count min/max/expected conditioned on the item dropping, the exact count distribution, and the signatures of any conditions the engine could not resolve. Fishing deliberately lives in `fishing-odds.json` instead. Both files are written by the normal `process` run, and `node scripts/build-odds.mjs <version>` rebuilds just these two from a workspace that has already been prepared, without downloading or decompiling anything. See [docs/CONSUMING_LOOT_ODDS.md](docs/CONSUMING_LOOT_ODDS.md)
